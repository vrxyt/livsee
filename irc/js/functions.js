...
function infobox_callback2(transport) {
	try {
		...
		$$("#infoBox .modal-content")[0].innerHTML=transport.responseText;
...
function rewrite_urls(s) {
	try {
		...
		if (s) {
			...
			var img_proxy = get_cookie("ttirc_imgproxy") == "true";

			tokens.each(function(u) {
				if (m = u.match(/([a-z]+:\/\/[^ ]+)/i)) {

					var url = m[1];

					if (img_proxy && u.match("\.(jpe?g|gif|png|bmp)$")) {
						url = "backend.php?op=imgproxy&url=" + encodeURIComponent(u);
					}

					if (img_proxy && u.match("\.(gifv|mp4|webm)$")) {
						url = "backend.php?op=vidproxy&url=" + encodeURIComponent(u);
					}

					escaped_str += u.replace(m[1],
						"<a target=\"_blank\" rel=\"noreferrer\" onclick=\"return url_clicked(this, event)\" " +
						"href=\""+url.replace(/"/g, '&quot;')+"\">"+m[1]+"</a>");
				} else {
					escaped_str += u;
				}
