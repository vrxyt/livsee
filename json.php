<?php
	...
	switch($_GET["action"])
	{
		...
		case "record":
			if (isset($_GET["start"])) {
				$json["data"]["file"] = exec('curl "https://localhost/control/record/start?app=live&name=' .$_GET["channel"]. '&rec=rec"');
			} else if (isset($_GET["stop"])) {
				$json["data"]["file"] = exec('curl "https://localhost/control/record/stop?app=live&name=' .$_GET["channel"]. '&rec=rec"');
			}
			break;
		default:
